...
import V18 as MinimalisticModel
...
modelVersion="V18_singleTrajectory_len12_40_ADAM_LOSS"
isTrainModel=True
continueTrain=True
...
varVal=np.std(selectedOrNotSelected,axis=1)*3.0
...
def train(X_trains, selectedOrNotSelecteds, serializedSelected2DMatrixs, BATCH_SIZE,optimizer_input,cri, offsetWeighting=0, maxWeightingStep=0.1, numIterates=30, extraIterates=0, beforeStepsFixed=0, isVisualizeLoss=False, isAdvancedWeighting=True, isAdvancedExponent=False):
    ...
    scheduler1 = ExponentialLR(optimizer_input, gamma=0.994)
    ...
    for i in bar:
        ...
        # optimizer_input.step()
        if i % 3 == 0:
            bar.set_description(f'loss: {epoch_loss/total:.5f}, lr: {scheduler1.get_last_lr()[0]:.5f}, ASV: {adjustedScheduleValue}')
        scheduler1.step()
