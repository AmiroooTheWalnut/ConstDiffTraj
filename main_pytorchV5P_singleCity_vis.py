...
import V6ConditionedModel as MinimalisticModel
...
modelVersion="V6ConditionalModel_SinlgCity_vis"
isTrainModel=False
continueTrain=True
isRunOnCPU=False
...
for c in range(numCities):
    ...
    #                                                                                  visualize=True)
    [data, nGrid, selectedOrNotSelected,serializedSelected2DMatrix] =DataGeneratorFcn.generateSyntheticDataFixedLengthInputImage("testStreets2.png",numTrajectories=numTrajectories,trajectoryLength=trajectoryLength,numGrid=40, seed=seeds[c],visualize=False)
    if isRunOnCPU == False:
...
def train(X_trains, selectedOrNotSelecteds, serializedSelected2DMatrixs, BATCH_SIZE,optim,cri, numIterates=30, isVisualizeLoss=False):
    bar = trange(numIterates)
    total = 30
    scheduler1 = ExponentialLR(optimizer, gamma=0.9991)
...
if isTrainModel==True:
    train(datas, selectedOrNotSelecteds, serializedSelected2DMatrixs, BATCH_SIZE, optimizer, criterion, numIterates=numOptIterates, isVisualizeLoss=True)
    model.eval()
...
def predict(serializedSelected2DMatrix, trajectoryLength,numTraj=10):
    ...
    fig, axs = plt.subplots(nrows=1, ncols=(int)(3))
    with torch.no_grad():
        ...
        for i in trange(timesteps):
            ...
            # visualiza(selectedOrNotSelected,numTraj, x.cpu().detach().numpy(), axis=axs[rowVal,colVal])
            # axs[rowVal,colVal].title.set_text("Time: "+str(i))
            if i>=13:
                visualiza(selectedOrNotSelected, numTraj, x.cpu().detach().numpy(), axis=axs[i-13])
    plt.show()
    return x
...
numPredicts = 2000
pred = predict(serializedSelected2DMatrix, trajectoryLength,numTraj=numPredicts)
