...
class SimpleNN(nn.Module):
    def __init__(self, forbiddenSerialMap=None, lenForbidden=10, maxLengthSize = 10, temporalFeatureSize=2, convOffset=0):
        ...
        self.lastAdd = Add3()
        self.lastDense = nn.Linear(self.size + 10, 64)
...
def generate_ts(timesteps, num, learningScheduleTime, isChangeWeights, isAdvancedWeighting=True):
    ...
    noisyOnes = np.random.randint(0, (int)(math.floor(timesteps * 0.4)), size=(int)(num / (0.9 + actualLearningScheduleTime * 20.0)))
    lastOnes = np.random.randint((int)(math.floor(timesteps * 0.4)), timesteps, size=(int)(num / (6.6-actualLearningScheduleTime*6.0)))
    lastOnes2 = np.random.randint((int)(math.floor(timesteps*0.8)), timesteps, size=(int)(num / (6.2-actualLearningScheduleTime*6.0)))
    lastOnes3 = np.random.randint((int)(math.floor(timesteps * 0.94)), timesteps, size=(int)(num / (6.05-actualLearningScheduleTime*6.0)))
...
def forward_noise_notNormalized(meanVals, varVals, timesteps, x, t, learningScheduleTime, isChangeWeights, isVizualize=True, isAdvancedWeighting=True):
    ...
    if isAdvancedWeighting == True:
        ...
        img_a = x * (1 - np.pow(a, 1.5 + learningScheduleTime * 5.0)) + noise * np.pow(a, 1.5 + learningScheduleTime * 5.0)
        img_b = x * (1 - np.pow(b, 1.5 + learningScheduleTime * 5.0)) + noise * np.pow(b, 1.5 + learningScheduleTime * 5.0)
    else:
...
class ConvLayers(nn.Module):
    def __init__(self, maxLengthSize,size,lenOffest,featureSize, convOffset=0):
        ...
        self.activation23 = nn.PReLU()
        self.activation24 = nn.PReLU()
        self.activation25 = nn.PReLU()
        self.activation26 = nn.PReLU()
        self.activation27 = nn.PReLU()
        ...
        self.conv3d1 = nn.Conv1d(self.size, self.maxLengthSize, 13+convOffset, padding=(int)(6+convOffset/2))
        ...
        self.conv3d2 = nn.Conv1d(self.size, self.maxLengthSize, 13+convOffset, padding=(int)((6+convOffset/2)*2), dilation=2)
        ...
        self.conv3d3 = nn.Conv1d(self.size, self.maxLengthSize, 13+convOffset, padding=(int)((6+convOffset/2)*3), dilation=3)
    ...
    def forward(self,input):
        ...
        # mergePath = mergePath.permute(0, 2, 1)
        mergePath1 = self.activation19(mergePath1)

        # mergePath = mergePath.permute(0, 2, 1)
        mergePath1 = self.conv2d1(mergePath1)
        # mergePath = mergePath.permute(0, 2, 1)
        mergePath1 = self.activation20(mergePath1)

        # mergePath = mergePath.permute(0, 2, 1)
        mergePath1 = self.conv3d1(mergePath1)
        # mergePath = mergePath.permute(0, 2, 1)
        mergePath1 = self.activation21(mergePath1)

        # mergePath = mergePath.permute(0, 2, 1)
        mergePath2 = self.conv1d2(input)
        # mergePath = mergePath.permute(0, 2, 1)
        mergePath2 = self.activation22(mergePath2)

        # mergePath = mergePath.permute(0, 2, 1)
        mergePath2 = self.conv2d2(mergePath2)
        # mergePath = mergePath.permute(0, 2, 1)
        mergePath2 = self.activation23(mergePath2)

        # mergePath = mergePath.permute(0, 2, 1)
        mergePath2 = self.conv3d2(mergePath2)
        # mergePath = mergePath.permute(0, 2, 1)
        mergePath2 = self.activation24(mergePath2)

        # mergePath = mergePath.permute(0, 2, 1)
        mergePath3 = self.conv1d3(input)
        # mergePath = mergePath.permute(0, 2, 1)
        mergePath3 = self.activation25(mergePath3)

        # mergePath = mergePath.permute(0, 2, 1)
        mergePath3 = self.conv2d3(mergePath3)
        # mergePath = mergePath.permute(0, 2, 1)
        mergePath3 = self.activation26(mergePath3)

        # mergePath = mergePath.permute(0, 2, 1)
        mergePath3 = self.conv3d3(mergePath3)
        # mergePath = mergePath.permute(0, 2, 1)
        mergePath3 = self.activation27(mergePath3)
