...
class PairwiseSubtractionLayer(nn.Module):
    ...
    def forward(self, A):
        ...
        #
        # # temp = torch.nn.CosineSimilarity(dim=2)
        # # dist=temp(torch.reshape(A,(A.shape[0],2,A.shape[1])),self.B.repeat((256,1,1)))
        #
        # return poolValue4
...
class SimpleNN(nn.Module):
    def __init__(self, forbiddenSerialMap=None, lenForbidden=10, maxLengthSize = 10, temporalFeatureSize=2):
        ...
        self.timeDense = nn.Linear(1, 64)
        self.forbDense = nn.Linear(2, 64)
        ...
        self.mult1 = Multiply()
        self.conv1d1 = nn.Conv1d(maxLengthSize*2+2, self.size, 11, padding=5)
        ...
        self.conv1d2 = nn.Conv1d(maxLengthSize*2+2, self.size, 11, padding=10, dilation=2)
        ...
        self.conv1d3 = nn.Conv1d(maxLengthSize*2+2, self.size, 11, padding=15, dilation=3)
        ...
        self.e11 = nn.Conv1d(maxLengthSize*2+2, 64, kernel_size=3, padding=1)  # output: 570x570x64
        ...
        self.denseAfterCat4 = nn.Linear(self.size * 4, self.size)
        self.denseAfterCat4_2 = nn.Linear(self.size * 4, self.size)
        self.lastAdd = Add3()
    ...
    def forward(self, traj, time, condLat, condLon):
        ...
        trajPathAfterCatForbTime = torch.cat((forbDensed,timeDensed,trajPath),dim=1)
        ...
        xe11 = self.activation(self.e11(trajPathAfterCatForbTime))
        ...
        mergePath1 = self.conv1d1(trajPathAfterCatForbTime)
        # mergePath = mergePath.permute(0, 2, 1)
        ...
        # mergePath = mergePath.permute(0, 2, 1)
        mergePath2 = self.conv1d2(trajPathAfterCatForbTime)
        # mergePath = mergePath.permute(0, 2, 1)
        ...
        # mergePath = mergePath.permute(0, 2, 1)
        mergePath3 = self.conv1d3(trajPathAfterCatForbTime)
        # mergePath = mergePath.permute(0, 2, 1)
        ...
        xd42 = self.lastDenseInPathAdjust(xd42)
        ...
        xd42 = torch.reshape(xd42, (traj.shape[0], self.maxLengthSize, -1))
        ...
        mergePath3Total = self.denseAfterCat4(mergePath3Total)
        ...
        finalPath = torch.cat((timePath, mergePath3Total, trajPathDirect,xd42), 2)
        finalPath = self.denseAfterCat4_2(finalPath)
...
def generate_ts(timesteps, num):
    ...
    lastOnes = np.random.randint((int)(math.floor(timesteps*0.5)), timesteps, size=(int)(num/1))
