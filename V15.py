...
def generate_ts(timesteps, num, learningScheduleTime, isChangeWeights, isAdvancedWeighting=True):
    ...
    lastOnes = np.random.randint((int)(math.floor(timesteps * 0.1)), timesteps, size=(int)(num / (7.0-actualLearningScheduleTime*6.0)))
    lastOnes2 = np.random.randint((int)(math.floor(timesteps*0.1)), timesteps, size=(int)(num / (6.8-actualLearningScheduleTime*6.0)))
    lastOnes3 = np.random.randint((int)(math.floor(timesteps * 0.1)), timesteps, size=(int)(num / (6.7-actualLearningScheduleTime*6.0)))
...
def forward_noise_notNormalized(meanVals, varVals, timesteps, x, t, learningScheduleTime, isChangeWeights, isVizualize=True, isAdvancedWeighting=True):
    ...
    if isAdvancedWeighting == True:
        ...
        img_a = x * (1 - np.pow(a, 2 + learningScheduleTime * 2.5)) + noise * np.pow(a, 2 + learningScheduleTime * 2.5)
        img_b = x * (1 - np.pow(b, 2 + learningScheduleTime * 2.5)) + noise * np.pow(b, 2 + learningScheduleTime * 2.5)
    else:
