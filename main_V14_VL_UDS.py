...
import V15 as MinimalisticModel
...
[data, nGrid, serializedSelected2DMatrix, selectedOrNotSelected]=DataGeneratorFcn.generateSyntheticDataFixedLengthInputImage("testStreets2.png", numTrajectories=numTrajectories,
...
varVal=np.std(selectedOrNotSelected,axis=1)*6.0
...
def train(X_trains, selectedOrNotSelecteds, serializedSelected2DMatrixs, BATCH_SIZE,optimizer_input,cri, offsetWeighting=0, maxWeightingStep=0.1, numIterates=30, extraIterates=0, beforeStepsFixed=0, isVisualizeLoss=False, isAdvancedWeighting=True, isAdvancedExponent=False):
    ...
    for i in bar:
        ...
        # print(adjustedScheduleValue)
        if optimResetCounter>numIterates*1.2:
