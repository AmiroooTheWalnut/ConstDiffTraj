...
def generate_ts(timesteps, num):
    ...
    lastOnes = np.random.randint((int)(math.floor(timesteps * 0.4)), timesteps, size=(int)(num / 1.6))
    lastOnes2 = np.random.randint((int)(math.floor(timesteps*0.8)), timesteps, size=(int)(num / 0.8))
    lastOnes3 = np.random.randint((int)(math.floor(timesteps * 0.93)), timesteps, size=(int)(num / 0.3))
    finalTs = np.concat((orig,lastOnes,lastOnes2,lastOnes3))
...
def forward_noise_notNormalized(meanVals, varVals, timesteps, x, t, learningScheduleTime, isChangeWeights, isVizualize=True, isAdvancedWeighting=True):
    ...
    if isAdvancedWeighting == True:
        ...
        img_a = x * (1 - np.pow(a, 6.2)) + noise * np.pow(a, 6.2)
        img_b = x * (1 - np.pow(b, 6.2)) + noise * np.pow(b, 6.2)
...
def forward_noise(timesteps, x, t, isVizualize=True):
    global oneTimeVisGenAB
    time_bar = 1 - np.linspace(0, 1.0, timesteps + 1)  # linspace for timesteps
    a = time_bar[t]  # base on t
    b = time_bar[t + 1]  # image for t + 1
    ...
    img_a = x * (1 - np.pow(a,2.8)) + noise * np.pow(a,2.8)
    img_b = x * (1 - np.pow(b,2.8)) + noise * np.pow(b,2.8)
