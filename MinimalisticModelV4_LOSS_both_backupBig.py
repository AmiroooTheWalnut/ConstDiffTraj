...
class CustomLoss(nn.Module):
    ...
    def custom_activation(self,x):
        a = torch.sigmoid(20*x)
        b = torch.sigmoid(-20*x)
        return a * b
    ...
    def forward(self, y_true, y_pred, time):
        ...
        return torch.max((torch.max(torch.max(mae_loss,dim=1).values,dim=1).values)*(16 - torch.squeeze(time) + 1)) + 0.0 * torch.sum(penalty)
...
class SimpleNN(nn.Module):
    def __init__(self, forbiddenSerialMap=None, lenForbidden=10, maxLengthSize = 10, temporalFeatureSize=2):
        ...
        self.psl = psl
        self.pslDense = nn.Linear(1, 1024)
        self.trajInput = nn.Linear(temporalFeatureSize, 128)
        self.trajInputDirect = nn.Linear(temporalFeatureSize, 128)
        self.timeInput = nn.Linear(1, maxLengthSize * 128)
        self.mult1 = Multiply()
        self.conv1d1 = nn.Conv1d(maxLengthSize, 256, 11, padding=5)
        self.activation = nn.Sigmoid()
        self.conv2d1 = nn.Conv1d(256, 256, 7, padding=3)
        self.conv3d1 = nn.Conv1d(256, 256, 5, padding=2)

        #self.auxDense = nn.Linear(510, 512)

        self.conv1d2 = nn.Conv1d(maxLengthSize, 256, 11, padding=10, dilation=2)
        self.conv2d2 = nn.Conv1d(256, 256, 7, padding=6, dilation=2)
        self.conv3d2 = nn.Conv1d(256, 256, 5, padding=4, dilation=2)

        self.conv1d3 = nn.Conv1d(maxLengthSize, 256, 11, padding=15, dilation=3)
        self.conv2d3 = nn.Conv1d(256, 256, 7, padding=9, dilation=3)
        self.conv3d3 = nn.Conv1d(256, 256, 5, padding=6, dilation=3)

        # self.activation = Activation()
        self.lastDenseInPath = nn.Linear(8192, 128)
        self.denseAfterCat3 = nn.Linear(128*3, 128)
        self.lastAdd = Add3()
        self.lastDense = nn.Linear(256 + 128, 512)
    ...
    def forward(self, traj, time):
        ...
        plsPathDRS = torch.reshape(plsPathD, (-1, self.maxLengthSize, (int)(1024 / self.maxLengthSize)))

        trajPath = self.trajInput(traj)
        trajPathDirect = self.trajInputDirect(traj)
        timePath = self.timeInput(time)
        timePath = torch.reshape(timePath, (-1, self.maxLengthSize, 128))
        ...
        mergePath1 = torch.reshape(mergePath1, (-1, self.maxLengthSize, 256 * 32))
        mergePath2 = torch.reshape(mergePath2, (-1, self.maxLengthSize, 256 * 32))
        mergePath3 = torch.reshape(mergePath3, (-1, self.maxLengthSize, 256 * 32))
