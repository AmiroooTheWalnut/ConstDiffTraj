...
import V21 as MinimalisticModel
...
# seed 19
tilesSynthetic = TilesSynthetic("datasets/SyntheticTiles/full.png",3,3)
allPaths,streetPoints,initLats, initLons, cutLats, cutLons = tilesSynthetic.makeSyntheticPaths(10,3,3,0.05,120,scale)
...
varVal=np.std(streetPoints,axis=1)*8.0


if isRunOnCPU == False:
    selectedOrNotSelected = torch.from_numpy(streetPoints).cuda()
else:
    selectedOrNotSelected = torch.from_numpy(streetPoints)
...
def predict(serializedSelected, trajectoryLength,numTraj=10):
    ...
    with torch.no_grad():
        ...
        for i in trange(timesteps):
            ...
            t = i/timesteps
            x_ts = np.pow(np.full((numTraj), t),6.0)
