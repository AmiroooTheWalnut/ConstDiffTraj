...
class SimpleNN(nn.Module):
    def __init__(self, forbiddenSerialMap=None, lenForbidden=10, maxLengthSize = 10, temporalFeatureSize=2, convOffset=0, batchSize=-1):
        ...
        self.forbDense = nn.Linear(2, self.sizeConvs)
        ...
        # self.pslDense = nn.Linear(1 + 2, self.maxLengthSize * 50)#replaced by embedding
        self.embeddingLatLonForbSum=nn.Embedding(40,3)
        # self.trajCondConcat=
        self.trajInput = nn.Linear(temporalFeatureSize, self.sizeConvs)
        # self.conditionInput = nn.Linear(2, 16)
        self.trajInputDirect = nn.Linear(temporalFeatureSize, self.size)
        # self.allCondsDense = nn.Linear(1 + 2, maxLengthSize * self.size)# replaced by embedding
        self.embeddingAllConds = nn.Embedding(40, 1)
        # self.mult1 = Multiply()

        self.ConvDirect = ConvLayers(maxLengthSize,self.sizeConvs,1,self.temporalFeatureSize,convOffset=convOffset)
        self.ConvDists = ConvLayers(maxLengthSize,self.sizeConvs,0,self.temporalFeatureSize,convOffset=convOffset)
    ...
    def forward(self, traj, time, condLat, condLon):
        ...
        plsPathD=self.embeddingLatLonForbSum(mixedPlsPathCond)
        ...
        # allCondsPath = self.allCondsDense(mixedTimeCond)# replaced by embedding
        allCondsPath=self.embeddingAllConds(mixedTimeCond)
        allCondsPath = torch.reshape(allCondsPath, (-1, self.maxLengthSize, self.size))
        ...
        allInfo = torch.cat((mergePathTraj2,mergePathForb2,allCondsPath),dim=2)
        ...
        finalPath = torch.cat((allCondsPath, allInfoDrnsed, trajPathDirect,mergePathTraj2,mergePathForb2), 2)
...
def forward_noise_notNormalized(meanVals, varVals, timesteps, x, t, learningScheduleTime, isChangeWeights, isVizualize=True, isAdvancedWeighting=True):
    ...
    if isAdvancedWeighting == True:
        ...
        img_a = x * (1 - np.pow(a, 7.0)) + noise * np.pow(a, 7.0)
        img_b = x * (1 - np.pow(b, 7.0)) + noise * np.pow(b, 7.0)
