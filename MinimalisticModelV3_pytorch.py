...
class SimpleNN(nn.Module):
    def __init__(self, forbiddenSerialMap=None, lenForbidden=10, maxLengthSize = 10, temporalFeatureSize=2):
        ...
        self.lastDenseInPath = nn.Linear(256, 256)
        self.lastAdd = Add3()
        self.lastDense = nn.Linear(64+256, 256)
        self.convLast = nn.Conv1d(256, 2, 1)

    def forward(self, traj, time):
        plsPath = self.psl(traj)
        plsPathD = self.pslDense(plsPath)
        plsPathDRS = torch.reshape(plsPathD, (-1, self.maxLengthSize, (int)(256/self.maxLengthSize)))
