...
import V11Revived as MinimalisticModel
...
modelVersion="V11ConditionalUNet_revived_singleCity_80"
isTrainModel=True
continueTrain=True
isRunOnCPU=False
...
def train_one(x_img,opt,cr):
    ...
    x_a, x_b = MinimalisticModel.forward_noise(timesteps, x_img, x_ts,isVizualize=True)
...
def train(X_trains, selectedOrNotSelecteds, serializedSelected2DMatrixs, BATCH_SIZE,optimizer_input,cri, numIterates=30, isVisualizeLoss=False):
    bar = trange(numIterates)
    total = 15
    scheduler1 = ExponentialLR(optimizer_input, gamma=0.9985)
    if isVisualizeLoss==True:
        lv=LossVisualizer(numIterates)
    optimResetCounter=0
    usingLR = initialLR
    for i in bar:
        if optimResetCounter>numIterates*0.2:
            usingLR=usingLR*0.8
        ...
        for j in range(total):
            ...
            if isCompleteRandomCities==True:
                ...
                loss = train_one(x_img, optimizer_input, cri)
