...
import V14_varLen_simpleRepeat as MinimalisticModel
...
modelVersion="V14VarLen_simpleRepeatLast_95_safegraph_tucson_7_4_4"
isTrainModel=True
continueTrain=True
...
def visualize_extent(selectedOrNotSelected,numInstances, input, selectedOrNotSelectedSerialized, axis=None, auxSelectedOrNotSelected=None):
    cmap_name = 'viridis'  # Example: Use the 'viridis' colormap
    cmap = cm.get_cmap(cmap_name, numInstances)
...
for r in range(0,data1.shape[0],2):
    ...
    counter = counter + 1
serializedSelected2DMatrix1=np.genfromtxt("datasets/tucson_allowableNodesV2_4_4.csv",delimiter=",",dtype=np.float32, filling_values=np.nan)
serializedSelected2DMatrix1=serializedSelected2DMatrix1.transpose()*scale
data=data2*scale
serializedSelected2DMatrix=serializedSelected2DMatrix1
selectedOrNotSelected=np.genfromtxt("datasets/tucson_allowableMatrix_4_4.csv",delimiter=",",dtype=np.float32, filling_values=np.nan)
nGrid=selectedOrNotSelected.shape[0]
maxTrajectoryLength=data.shape[1]



meanVal=np.mean(serializedSelected2DMatrix,axis=1)
varVal=np.var(serializedSelected2DMatrix,axis=1)*6.0
...
def train(X_trains, selectedOrNotSelecteds, serializedSelected2DMatrixs, BATCH_SIZE,optimizer_input,cri, offsetWeighting=0, maxWeightingStep=0.1, numIterates=30, extraIterates=0, beforeStepsFixed=0, isVisualizeLoss=False, isAdvancedWeighting=True, isAdvancedExponent=False):
    ...
    for i in bar:
        ...
        if optimResetCounter>numIterates*0.34:
            usingLR=usingLR*0.85
            optimizer_input = optim.Adam(model.parameters(), lr=usingLR)
            scheduler1 = ExponentialLR(optimizer_input, gamma=0.995)
            optimResetCounter=0
...
if isTrainModel==True:
    ...
    for r in range(repeatTimes):
        offsetValues = np.arange(np.minimum(fixedOffset+(r/repeatTimes)*maxOffsetMovement,0.99), 1, weightingStepSize)
        print(offsetValues)
...
for c in range(numCities):
    pred = predict(serializedSelected2DMatrixs[c], maxTrajectoryLength,numTraj=numPredicts)

    pred = pred.cpu().detach().numpy()

    JSDValue = JSD(nGrid,datas[c],pred,selectedOrNotSelecteds[c], scale=scale)
    ...
    visualize_extent(selectedOrNotSelecteds[c],numPredicts, pred, serializedSelected2DMatrixs[c])
...
visualize_extent(selectedOrNotSelectedT,numPredicts, pred, serializedSelected2DMatrixT, auxSelectedOrNotSelected=mainSelectedOrNotSelected)
