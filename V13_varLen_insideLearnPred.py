...
class CustomLoss(nn.Module):
    ...
    def forward(self, y_pred, y_true, time, trainMask, obtained_mask):
        mae_loss = torch.abs(y_true - y_pred)

        mae_loss = mae_loss*obtained_mask
        ...
        C = C * obtained_mask.unsqueeze(dim=3).repeat(1, 1, 1, self.lenForbidden)
        ...
        maskPen = torch.mean(torch.abs(trainMask - obtained_mask))

        # return torch.mean(mae_loss) + torch.mean(torch.abs(penaltyTrue-penaltyPred)) + maskPen
        return torch.mean(mae_loss)+maskPen*1.0
...
class PairwiseSubtractionLayer(nn.Module):
    ...
    def forward(self, A, mask):
        ...
        mask1 = mask * self.maxLengthSize
        if mask1.device.type=='cuda':
            mask2 = nn.Parameter(torch.arange(self.maxLengthSize, dtype=torch.float32).repeat(mask.shape[0], 1).cuda())
        else:
            mask2 = torch.arange(self.maxLengthSize).repeat(mask.shape[0], 1)

        mask3 = nn.Parameter((mask1>mask2).float())
        mask4 = mask3.unsqueeze(dim=2).repeat(1, 1, 2)
        C = C * mask4.unsqueeze(dim=3).repeat(1, 1, 1, self.lenForbidden)
...
class PairwiseSubtractionSumLayer(nn.Module):
    ...
    def forward(self, A, mask):
        ...
        mask1 = mask * self.maxLengthSize
        if mask1.device.type == 'cuda':
            mask2 = torch.arange(self.maxLengthSize).repeat(mask.shape[0], 1).cuda()
        else:
            mask2 = torch.arange(self.maxLengthSize).repeat(mask.shape[0], 1)

        mask3 = (mask1 > mask2).int()
        mask4 = mask3.unsqueeze(dim=2).repeat(1, 1, 2)
        C = C * mask4.unsqueeze(dim=3).repeat(1, 1, 1, self.lenForbidden)
...
class SimpleNN(nn.Module):
    def __init__(self, forbiddenSerialMap=None, lenForbidden=10, maxLengthSize = 10, temporalFeatureSize=2):
        ...
        self.ConvDirectThresh = ConvLayers(maxLengthSize, self.size, 0, self.temporalFeatureSize)

        self.lengthDenseNoSeq = nn.Linear(8, 1)
        self.lengthDense = nn.Linear(12+3, self.size)
        self.lengthDense2 = nn.Linear(self.size, 1)
        self.activationLength = nn.Hardsigmoid()
        ...
        self.ConvDirect = ConvLayers(maxLengthSize,self.size,1,self.temporalFeatureSize)
        self.ConvDists = ConvLayers(maxLengthSize,self.size,0,self.temporalFeatureSize)
    ...
    def forward(self, traj, time, condLat, condLon):
        densedTraj = self.trajInput(traj)
        threshConv = self.ConvDirectThresh(densedTraj)
        threshConv2 =self.lengthDenseNoSeq(threshConv)
        threshConv2 = threshConv2.squeeze()
        allLength = torch.concat((threshConv2,condLat,condLon,time),dim=1)
        threshDense1 = self.lengthDense(allLength)
        threshDense2 = self.lengthDense2(threshDense1)
        threshDense3 = self.activationLength(threshDense2)

        plsPath, mask1 = self.psl(traj,threshDense3)
        plsPathSum, mask2 = self.pslSum(traj,threshDense3)
...
def generate_ts(timesteps, num, learningScheduleTime, isChangeWeights, isAdvancedWeighting=True):
    ...
    noisyOnes = np.random.randint(0, (int)(math.floor(timesteps * 0.4)),
                                 size=(int)(num / (0.9 + actualLearningScheduleTime * 20.0)))
    lastOnes = np.random.randint((int)(math.floor(timesteps * 0.4)), timesteps, size=(int)(num / (6.9-actualLearningScheduleTime*6.0)))
    lastOnes2 = np.random.randint((int)(math.floor(timesteps*0.8)), timesteps, size=(int)(num / (6.6-actualLearningScheduleTime*6.0)))
    lastOnes3 = np.random.randint((int)(math.floor(timesteps * 0.94)), timesteps, size=(int)(num / (6.2-actualLearningScheduleTime*6.0)))
...
def forward_noise(timesteps, x, t, learningScheduleTime, isChangeWeights, isVizualize=True, isAdvancedWeighting=True):
    ...
    if isAdvancedWeighting == True:
        ...
        img_a = x * (1 - np.pow(a, 1.5+learningScheduleTime*2.8)) + noise * np.pow(a, 1.5+learningScheduleTime*2.8)
        img_b = x * (1 - np.pow(b, 1.5+learningScheduleTime*2.8)) + noise * np.pow(b, 1.5+learningScheduleTime*2.8)
    else:
...
class ConvLayers(nn.Module):
    def __init__(self, maxLengthSize,size,lenOffest,featureSize):
        ...
        self.conv3d1 = nn.Conv1d(self.size, self.maxLengthSize, 7, padding=3)
        ...
        self.conv3d2 = nn.Conv1d(self.size, self.maxLengthSize, 7, padding=6, dilation=2)
        ...
        self.conv3d3 = nn.Conv1d(self.size, self.maxLengthSize, 7, padding=9, dilation=3)
