...
def generate_ts(timesteps, num):
    ...
    lastOnes = np.random.randint((int)(math.floor(timesteps * 0.4)), timesteps, size=(int)(num / 3.0))
    lastOnes2 = np.random.randint((int)(math.floor(timesteps*0.8)), timesteps, size=(int)(num / 2.2))
    lastOnes3 = np.random.randint((int)(math.floor(timesteps * 0.93)), timesteps, size=(int)(num / 1.0))
...
def forward_noise(timesteps, x, t, isVizualize=True):
    ...
    img_a = x * (1 - np.pow(a,3.5)) + noise * np.pow(a,3.5)
    img_b = x * (1 - np.pow(b,3.5)) + noise * np.pow(b,3.5)
