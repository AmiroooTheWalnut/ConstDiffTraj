...
import V13_varLen_insideLearnPred as MinimalisticModel
...
modelVersion="V13VarLen_insideLearnPred_80_LOSS"
isTrainModel=True
continueTrain=True
...
def visualiza(selectedOrNotSelected,numInstances, input, in_mask, axis=None, auxSelectedOrNotSelected=None):
...
for c in range(numCities-1):
    ...
    [data, mask, nGrid, selectedOrNotSelected, serializedSelected2DMatrix, _]=DataGeneratorFcn.generateSyntheticDataVariableLength(numTrajectories=numTrajectories,
...
model = MinimalisticModel.SimpleNN(forbiddenSerialMap=serializedSelected2DMatrix,lenForbidden=serializedSelected2DMatrix.shape[1],maxLengthSize=maxTrajectoryLength,temporalFeatureSize=2)
...
# maskTemp=torch.randn(BATCH_SIZE,maxTrajectoryLength,2)
# Forward pass
if isRunOnCPU==False:
    model=model.cuda()
    in1=in1.cuda()
...
def train_one(x_img,train_mask,opt,cr,learningScheduleTime, isAdvancedWeighting=True, isAdvancedExponent=False):
    ...
    if isRunOnCPU == False:
        ...
        if "LOSS" in modelVersion:
            loss = cr(outputs[0], x_b.cuda(), x_ts.cuda(), train_mask.cuda(), outputs[1])
        else:
            loss = cr(outputs[0], x_b.cuda())
    else:
        outputs = model(x_a, x_ts, cond1, cond2)
        if "LOSS" in modelVersion:
            loss = cr(outputs[0], x_b, x_ts, train_mask, outputs[1])
        else:
            loss = cr(outputs[0], x_b)
...
def train(X_trains, masks, selectedOrNotSelecteds, serializedSelected2DMatrixs, BATCH_SIZE,optimizer_input,cri, offsetWeighting=0, maxWeightingStep=0.1, numIterates=30, extraIterates=0, beforeStepsFixed=0, isVisualizeLoss=False, isAdvancedWeighting=True, isAdvancedExponent=False):
    ...
    for i in bar:
        ...
        for j in range(total):
            ...
            if isCompleteRandomCities==True:
                ...
                [data, mask, nGrid, selectedOrNotSelected,
                 serializedSelected2DMatrix,_] = DataGeneratorFcn.generateSyntheticDataVariableLength(
                ...
                loss = train_one(x_img, mask, optimizer_input, cri, adjustedScheduleValue,isAdvancedWeighting=isAdvancedWeighting,isAdvancedExponent=isAdvancedExponent)
                epoch_loss += loss.item() * 1
            else:
                for d in range(numCitiesTrain):
                    randIndex = np.random.randint(len(X_trains))
                    X_train = X_trains[randIndex]
                    maskD = masks[randIndex]
                    # model.sforbiddenSerialMap = selectedOrNotSelecteds[d]
                    ...
                    x_mask = maskD[batchIndices]
                    loss = train_one(x_img, x_mask, optimizer_input, cri, adjustedScheduleValue,isAdvancedWeighting=isAdvancedWeighting,isAdvancedExponent=isAdvancedExponent)
...
for c in range(numCities):
    pred, mask_output = predict(serializedSelected2DMatrixs[c], maxTrajectoryLength,numTraj=numPredicts)
    ...
    visualiza(selectedOrNotSelecteds[c],numPredicts, pred, mask_output)
...
[data, mask, nGrid, selectedOrNotSelectedT, serializedSelected2DMatrixT]=DataGeneratorFcn.generateSyntheticDataVariableLengthInputImage("testStreets3.png", numTrajectories=numTrajectories,
                                                                maxTrajectoryLength=maxTrajectoryLength, numGrid=40,
                                                                seed=123, visualize=False)
...
numPredicts = 800
pred, mask_output = predict(serializedSelected2DMatrixT, maxTrajectoryLength,numTraj=numPredicts)
...
visualiza(selectedOrNotSelectedT,numPredicts, pred, mask_output, auxSelectedOrNotSelected=mainSelectedOrNotSelected)
