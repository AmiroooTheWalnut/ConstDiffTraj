...
modelVersion="V14VarLen_simpleRepeatLast_95_synth_long"
isTrainModel=True
continueTrain=True
...
[data, nGrid, selectedOrNotSelected, serializedSelected2DMatrix]=DataGeneratorFcn.generateSyntheticDataVariableLengthInputImageLastRepeatDebug("testStreets2.png", numTrajectories=numTrajectories,
                                                                maxTrajectoryLength =maxTrajectoryLength, numGrid=40,
                                                                seed=12, visualize=True)

# data1=np.genfromtxt("datasets/tucson_mobilityJunctionSampleV3_1_2.csv",delimiter=",",dtype=np.float32, filling_values=np.nan)
...
#     counter = counter + 1
# serializedSelected2DMatrix1=np.genfromtxt("datasets/tucson_allowableNodesV2_1_2.csv",delimiter=",",dtype=np.float32, filling_values=np.nan)
# serializedSelected2DMatrix1=serializedSelected2DMatrix1.transpose()
# data=data2
# serializedSelected2DMatrix=serializedSelected2DMatrix1
# selectedOrNotSelected=np.genfromtxt("datasets/tucson_allowableMatrix_1_2.csv",delimiter=",",dtype=np.float32, filling_values=np.nan)
...
def train_one(x_img,opt,cr,learningScheduleTime, isAdvancedWeighting=True, isAdvancedExponent=False):
    ...
    x_a, x_b = MinimalisticModel.forward_noise(timesteps, x_img, x_ts, learningScheduleTime, isChangeWeights,isVizualize=True,isAdvancedWeighting=isAdvancedExponent)
